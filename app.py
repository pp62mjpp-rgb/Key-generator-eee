...
if __name__ == '__main__':
    import subprocess, sys, threading, time, re

    # ── AUTO INSTALL PACKAGES ──────────────────────────────
    def auto_install():
        pkgs = ['flask', 'requests', 'cloudflared']
        for pkg in pkgs:
            try:
        ...
        if result.returncode != 0:
            ...
            if system == 'linux':
                ...
                subprocess.call(['wget', '-q', url, '-O', '/usr/local/bin/cloudflared'])
                subprocess.call(['chmod', '+x', '/usr/local/bin/cloudflared'])
            elif system == 'darwin':
                subprocess.call(['brew', 'install', 'cloudflared'])
            elif system == 'windows':
                print("[!] Windows pe manually install karo:")
        ...
        try:
            ...
            for line in proc.stdout:
                ...
                if urls:
                    ...
                    print(f"  Share karo   : {urls[0]}")
                    print(f"{'='*55}\n")
        except FileNotFoundError:
            print("\n[!] cloudflared nahi mila — URL generate nahi hoga.\n")
        except Exception as e:
            print(f"\n[!] Tunnel error: {e}\n")

    threading.Thread(target=start_tunnel, args=(5000,), daemon=True).start()
